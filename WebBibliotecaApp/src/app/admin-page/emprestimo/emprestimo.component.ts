...
export class EmprestimoComponent implements OnInit {
  ...
  onSelectedRowsLivro(rows: Array<GuiSelectedRow>): void {
    this.formulario.patchValue({ livro: rows.map((m: GuiSelectedRow) => m.source.tiTulo)[0] });
    this.CodigoLivro = rows.map((m: GuiSelectedRow) => m.source.codLivro)[0];
  }
  onSelectedRowsLeitor(rows: Array<GuiSelectedRow>): void {
    this.formulario.patchValue({ leitor: rows.map((m: GuiSelectedRow) => m.source.nome)[0] });
  ...
  onSelectedRowsEmprestimo(rows: Array<GuiSelectedRow>): void {
    ...
    if (rows.length == 0) {
      ...
      this.formulario.patchValue({ leitor: rows.map((m: GuiSelectedRow) => m.source.leitor)[0] });
      this.formulario.patchValue({ livro: rows.map((m: GuiSelectedRow) => m.source.livro)[0] });
    }
  ...
  onSubmit() {
    ...
    this.SalvarEmprestimo(new Emprestimo(form.codEmprestimo, form.leitor, "0", form.livro, 0, form.dataEmprestimo, form.dataDevolucao, ""))
    this.emprestimoService.GetEmprestimo().subscribe(Emprestimos => { this.source = Emprestimos })
